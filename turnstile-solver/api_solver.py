...
class TurnstileAPIServer:
    ...
    def __init__(self, headless: bool, useragent: Optional[str], debug: bool, browser_type: str, thread: int, proxy_support: bool, use_random_config: bool = False, browser_name: Optional[str] = None, browser_version: Optional[str] = None, lazy_browser: bool = False):
        self.app = Quart(__name__)
        self.debug = debug
        self.browser_type = browser_type
        self.headless = headless
        self.thread_count = max(1, thread)
        self.proxy_support = proxy_support
        self.browser_pool = asyncio.Queue()
        self.solve_semaphore = asyncio.Semaphore(self.thread_count)
        self.use_random_config = use_random_config
        self.browser_name = browser_name
        self.browser_version = browser_version
        self.lazy_browser = lazy_browser
        self.lazy_browser_counter = 0
        self.browser_proxy = os.getenv("TURNSTILE_BROWSER_PROXY") or os.getenv("HTTPS_PROXY") or os.getenv("HTTP_PROXY")
    ...
    async def _startup(self) -> None:
        ...
        try:
            ...
            logger.warning(f"Skipping welcome output due to console encoding: {str(e)}")
        if self.lazy_browser:
            logger.info("Lazy browser mode enabled; browsers start only while solving")
        else:
            logger.info("Starting browser initialization")
        try:
            await init_db()
            if not self.lazy_browser:
    ...
    def _select_browser_config(self):
        if self.browser_type in ['chromium', 'chrome', 'msedge']:
    ...
    def _browser_launch_args(self, config):
        browser_args = [
            "--window-position=0,0",
        ...
        if self.browser_proxy:
            browser_args.append(f"--proxy-server={self.browser_proxy}")
        return browser_args
    ...
    async def _launch_browser(self, config):
        ...
        if self.browser_type in ['chromium', 'chrome', 'msedge']:
            ...
            browser = await playwright.chromium.launch(
                ...
                args=self._browser_launch_args(config)
            )
    ...
    async def _acquire_browser(self):
        if not self.lazy_browser:
            ...
            return index, browser, config, None

        self.lazy_browser_counter += 1
        index = self.lazy_browser_counter
        config = self._select_browser_config()
        browser, runtime = await self._launch_browser(config)
        if self.debug:
            logger.debug(f"Browser {index}: Lazy browser started with {config['browser_name']} {config['browser_version']}")
        return index, browser, config, runtime

    async def _release_browser(self, index, browser, config, runtime):
        if self.lazy_browser:
            try:
                await browser.close()
                if self.debug:
                    logger.debug(f"Browser {index}: Lazy browser closed")
            except Exception as e:
                if self.debug:
                    logger.warning(f"Browser {index}: Error closing lazy browser: {str(e)}")

            playwright = runtime.get('playwright') if runtime else None
            if playwright:
                try:
                    await playwright.stop()
                except Exception as e:
                    if self.debug:
                        logger.warning(f"Browser {index}: Error stopping Playwright: {str(e)}")
            return
    ...
    async def _initialize_browser(self) -> None:
        ...
        for i in range(self.thread_count):
            ...
            browser_args = self._browser_launch_args(config)
    ...
    async def _solve_turnstile(self, task_id: str, url: str, sitekey: str, action: Optional[str] = None, cdata: Optional[str] = None):
        ...
        await self.solve_semaphore.acquire()
        try:
            index, browser, browser_config, browser_runtime = await self._acquire_browser()
        except Exception as e:
            self.solve_semaphore.release()
            logger.error(f"Failed to acquire browser: {str(e)}")
        ...
        try:
            if hasattr(browser, 'is_connected') and not browser.is_connected():
                ...
                await self._release_browser(index, browser, browser_config, browser_runtime)
                self.solve_semaphore.release()
                await save_result(task_id, "turnstile", {"value": "CAPTCHA_FAIL", "elapsed_time": 0})
                return
        except Exception as e:
...
def parse_args():
    ...
    parser.add_argument('--lazy-browser', action='store_true', help='Start a browser only while a solve task is running')
...
def create_app(headless: bool, useragent: str, debug: bool, browser_type: str, thread: int, proxy_support: bool, use_random_config: bool, browser_name: str, browser_version: str, lazy_browser: bool) -> Quart:
    server = TurnstileAPIServer(headless=headless, useragent=useragent, debug=debug, browser_type=browser_type, thread=thread, proxy_support=proxy_support, use_random_config=use_random_config, browser_name=browser_name, browser_version=browser_version, lazy_browser=lazy_browser)
    return server.app
...
if __name__ == '__main__':
    ...
    if args.browser_type not in browser_types:
        ...
        app = create_app(
            ...
            lazy_browser=args.lazy_browser
        )
